...
bool Action::operator==(const sf::Event &event) const
{
    ...
    switch(event.type)
    {
        ...
        {
            if(_type & Type::Released && event.type == sf::Event::EventType::KeyReleased)
                res = event.key.code == _event.key.code;
        }break;
        case sf::Event::EventType::MouseButtonPressed:
        {
            if(_type & Type::Pressed && event.type == sf::Event::EventType::MouseButtonPressed)
                res = event.mouseButton.button == _event.mouseButton.button;
        }break;
        case sf::Event::EventType::MouseButtonReleased:
        {
            if(_type & Type::Released && event.type == sf::Event::EventType::MouseButtonReleased)
                res = event.mouseButton.button == _event.mouseButton.button;
        }break;
        default: break;
    }
